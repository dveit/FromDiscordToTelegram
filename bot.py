...
async def start(message: types.Message):
    if not BotDB.user_exists(message.from_user.id):
        ...
        await bot.send_message(message.from_user.id,
                               ...
                               "https://gist.github.com/9sv/3272bc9a63d64b1eb93286ffb3217130")
    else:
...
async def getting_channel_description(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        BotDB.add_channel(message.from_user.id, int(data['server_id']), data['server_name'], int(data['channel_id']),
                          data['channel_name'], data['tracked_users'], data['ignored_users'], data['last_message_id'])
...
async def show_tracked_channels(message: types.Message):
    ...
    for tpl in tracked_channels:
        ...
        if len(tracked_users) == 0:
            ...
            tracked_by_users.append(
                f'[{db_id}] {server_name} -> {channel_name} [✅{tracked_users_count}] [🚫{ignored_users_count}]')
...
async def delete_channel(message: types.Message):
    ...
    await bot.send_message(message.chat.id, "Send me DB index(db_id) of channel that you want to delete")
...
async def get_servername(ds_token: str, ds_server_id, ds_user_id) -> str:
    ...
    try:
        username_on_server = list(filter(lambda item: item['id'] == str(ds_server_id), json_data['mutual_guilds']))[0][
            'nick']
...
async def get_discord_messages(ds_token: str, ds_channel_id: int, last_message_id: int) -> list:
    ...
    try:
        r = requests.get(
            f"https://discord.com/api/v9/channels/{ds_channel_id}/messages?limit={100}&after={last_message_id}",
            headers=headers)
            ...
            for raw_message in data:
                ...
                if len(ignored_users) == 0 or author_id not in ignored_users:
                    ...
                    if 'referenced_message' in raw_message:
                        message.append("\n")
                        referenced_author_name = await get_servername(ds_token, server_id,
                                                                      raw_message['referenced_message']['author']['id'])
