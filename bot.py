...
async def getting_db_id(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        if data["db_id"].isdigit():
            ...
            await bot.send_message(chat_id=message.chat.id,reply_markup=ReplyKeyboardRemove(),text="Wrong DB ID. Edition canceled.")
            await state.finish()
...
async def getting_renaming_option(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        if data["renaming_option"] == "Server" or data["renaming_option"] == "Channel":
            await RenamingForm.next()
            await bot.send_message(chat_id=message.chat.id,reply_markup=ReplyKeyboardRemove(),text="Send me a new name")
        else:
            await bot.send_message(chat_id=message.chat.id,reply_markup=ReplyKeyboardRemove(),text="Wrong option selected. Edition canceled.")
            await state.finish()
...
async def getting_new_name(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        if data["renaming_option"] == "Server":
            ...
            await bot.send_message(chat_id=message.chat.id,reply_markup=ReplyKeyboardRemove(),text="Server name was updated!")
        else:
            BotDB.update_channel_name(message.from_user.id, data["db_id"], data["new_name"])
            await bot.send_message(chat_id=message.chat.id,reply_markup=ReplyKeyboardRemove(),text="Channel name was updated!")
    await state.finish()
    ...
    async with state.proxy() as data:
        ...
        if data["selected_option"] == "Discord token":
            ...
            await bot.send_message(
                message.from_user.id, "Send me Discord token.\nDon't know how to get it?\nWatch here (PC required) https://gist.github.com/dveit/09781b473a3f60a15e92a6a313985075")
    ...
    async with state.proxy() as data:
        if len(message.text) == 18 and (message.text).isdigit():
            ...
            if channel_name:
                ...
                await bot.send_message(message.chat.id, "Channel added!\n\n You can use /rename command to change servername or channel name as you wish.")
            else:
...
async def delete_channel(message: types.Message):
    ...
    await ChannelDeletetionForm.channel_id.set()
    await bot.send_message(message.chat.id, "Send me DB index([value]) of channel thah you want to delete")


@dp.message_handler(state=ChannelDeletetionForm.channel_id)
...
async def is_token_valid(ds_token: str) -> str:
...
async def get_discord_messages(
    ds_token: str, ds_channel_id: int, last_message_id: int) -> list:
...
async def processing(tg_user_id: int, ds_token: str, tz_delta: int):
    ...
    for row in tracked_list:
        ...
        if await is_token_valid(ds_token):
            ...
            token_error_message = []
            token_error_message.append(f"⚠️ Warning! ⚠️")
            token_error_message.append("\n")
            token_error_message.append("Discord token is invalid!")
