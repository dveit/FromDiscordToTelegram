...
class BotDB:
    ...
    def add_user(self, tg_user_id, ds_token, tz_delta):
        self.cursor.execute("INSERT INTO `bot_users` (`tg_user_id`, `ds_token`, `tz_delta`) VALUES (?, ?, ?)",
    ...
    def delete_channel(self, tg_user_id, db_id):
        self.cursor.execute("DELETE FROM `tracked_channels` WHERE `tg_user_id` = ? AND `id` = ? ", (tg_user_id, db_id,))
        return self.conn.commit()
    ...
    def get_tracked_channels(self, tg_user_id):
        result = self.cursor.execute(
            "SELECT `id`, `server_id`, `server_name`, `channel_id`, `channel_name`, `tracked_users`, `ignored_users`, "
            "`last_message_id` FROM `tracked_channels` WHERE `tg_user_id` = ?",
            (tg_user_id,))
        return result.fetchall()

    def get_channel_info(self, db_id, tg_user_id):
        result = self.cursor.execute(
            "SELECT `server_id`, `server_name`, `channel_id`, `channel_name`, `tracked_users`, `ignored_users`, "
            "`last_message_id` FROM `tracked_channels` WHERE `id` = ? AND `tg_user_id` = ?",
            (db_id, tg_user_id,))
        return result.fetchall()
    ...
    def get_tracked_users(self, db_id, tg_user_id):
        result = self.cursor.execute(
            "SELECT `tracked_users` FROM `tracked_channels` WHERE `id` = ? AND `tg_user_id` = ?", (db_id, tg_user_id,))
        return result.fetchall()

    def get_ignored_users(self, db_id, tg_user_id):
        result = self.cursor.execute(
            "SELECT `ignored_users` FROM `tracked_channels` WHERE `id` = ? AND `tg_user_id` = ?", (db_id, tg_user_id,))
        return result.fetchall()
    ...
    def update_tracked_user(self, db_id, tg_user_id, cell_content):
        self.cursor.execute("UPDATE `tracked_channels` SET `tracked_users` = ? WHERE `id` = ? AND `tg_user_id` = ?",
                            (cell_content, db_id, tg_user_id,))
        return self.conn.commit()

    def update_ignored_user(self, db_id, tg_user_id, cell_content):
        self.cursor.execute("UPDATE `tracked_channels` SET `ignored_users` = ? WHERE `id` = ? AND `tg_user_id` = ?",
                            (cell_content, db_id, tg_user_id,))
        return self.conn.commit()

    def update_message_id(self, tg_user_id, channel_id, new_message_id):
        self.cursor.execute(
            "UPDATE `tracked_channels` SET `last_message_id` = ? WHERE `tg_user_id` = ? AND `channel_id` = ?",
            (new_message_id, tg_user_id, channel_id,))
        return self.conn.commit()
